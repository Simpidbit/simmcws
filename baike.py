...
def baike(message, client):
	ensure_future(_baike(message, client))

async def _baike(message, client):
	search_url = "https://baike.baidu.com/search/word?word={}"
	...
	try:
		search_word = message[1]
	except(IndexError):
		await client.send(command(alert("您未输入任何关键词！")))
		return

	search_url = search_url.format(search_word)
	await client.send("发送GET请求...")
	...
	style = "tellraw @a {\"rawtext\":[{\"text\":\">> §a百度百科§f << "+search_result+" §e\"}]}"
	await client.send(command(style))
