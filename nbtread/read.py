...
async def _nbtfile(client, message, global_var):
	try:
		args = cmdarg.Cmd(message)
		open(args.args["-p"],"a").close()
		file_size = os.path.getsize(args.args["-p"])
...
def nbtfile(client, message, global_var):
	asyncio.ensure_future(_nbtfile(client, message, global_var))
